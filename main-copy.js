...
function like_button() {
    ...
    else { like.className = "bi bi-heart-fill" }
}
...
function repeat_music() {
    repeat_button()
    if (repeat.className == "bi bi-repeat-1" && time_progress() > 3) {
        alert()
    }
}
addEventListener("loaad", repeat_music)
...
function time_progress(param_progress) {
    ...
    else if (initial_time.innerText == 100 && repeat.className === "bi bi-repeat-1") {
        Music.play()
...
function Time_container() {
    initial_time.innerText = `${Math.floor(time_progress())}`
    final_time.innerText = 100
